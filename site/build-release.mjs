...
Options:
  --base-path <path>  Public URL base path for the uploaded docs bundle.
                      Examples: /, /docs/, /random/clio-kb/, auto
...
function getDeploymentBasePath(basePath) {
  return basePath === "auto" ? "/clio-kb/" : basePath;
}
  ...
  while (queue.length > 0) {
    ...
    for (const rawHref of collectMarkdownLinks(markdown)) {
      ...
      if (href.endsWith(".md")) {
        ...
        if (stat.isFile()) {
          assetFiles.add(resolvedPath);
        }
      } else {
        warnings.push(`Missing linked asset file: ${path.relative(process.cwd(), resolvedPath)}`);
      }
    }
  }

  return { markdownFiles, assetFiles, warnings };
}
...
function buildNginxConfig(basePath) {
  ...
  const placeholderComment = basePath === "auto"
    ? "# Replace /clio-kb/ with the public mount path for this bundle before using this snippet.\n"
    : "";
  return `${placeholderComment}# Clio KB static docs SPA
...
\`\`\``
  ...
  return `# Clio KB Release Deployment
...
async function main() {
  ...
  const { markdownFiles, assetFiles, warnings } = await collectReleaseFiles(sourceNav);
  ...
  const sortedAssetFiles = [...assetFiles].sort((left, right) => left.localeCompare(right));
  for (const assetPath of sortedAssetFiles) {
    await copyFileIntoRelease(assetPath, options.outDir);
  }
  ...
  if (sortedAssetFiles.length > 0) {
    console.log(`Copied ${sortedAssetFiles.length} linked asset files.`);
  }
  if (await pathExists(screenshotsSourceDir)) {
    console.log("Copied screenshot assets.");
  }
  if (options.basePath === "auto") {
    console.warn("Warning: --base-path auto is less robust for deployed subdirectory hosting. Prefer an explicit path such as /random/clio-kb/.");
  }
