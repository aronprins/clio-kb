...
(function() {
  const saved = localStorage.getItem('clio-kb-theme')
...
let metaRowObserver = null;
const RELEASE_BASE_PATH = "/clio-kb/";
let APP_BASE_PATH = "/";
...
document.getElementById('theme-toggle').addEventListener('click', () => {
  ...
  localStorage.setItem('clio-kb-theme', next);
...
async function init() {
  ...
  try {
    ...
    if (!isNavPayload(navData)) {
      throw new Error("content.json did not match the expected docs schema.");
    }
  } catch (e) {
...
function buildLanding() {
  ...
  grid.innerHTML = '';
  const sectionCount = navData.sections.length;
  const gridColumns = sectionCount === 4 ? 2 : Math.min(sectionCount || 1, 3);
  ...
  ql.innerHTML = '';
  const candidates = [
    allPages.find(p => /overview/i.test(p.file)),
    allPages.find(p => /quickstart/i.test(p.file)),
    allPages.find(p => /project-structure/i.test(p.file)),
  ].filter(Boolean);
...
const DOCS_REPO_SLUG = 'aronprins/clio-kb';
...
function setupStickyMetaRow(article) {
  if (metaRowObserver) {
    metaRowObserver.disconnect();
    metaRowObserver = null;
  }

  const row = article.querySelector('.meta-row');
  if (!row) return;

  const sentinel = document.createElement('div');
  sentinel.className = 'meta-row-sentinel';
  row.before(sentinel);

  const rootStyles = getComputedStyle(document.documentElement);
  const headerHeight = Number.parseFloat(rootStyles.getPropertyValue('--header-h')) || 64;
  const stickyOffset = headerHeight - 1;

  metaRowObserver = new IntersectionObserver(
    ([entry]) => {
      row.classList.toggle('is-stuck', !entry.isIntersecting);
    },
    {
      threshold: 0,
  ...
  metaRowObserver.observe(sentinel);
}
